...
const NavItem = ({ label, icon, path }) => (
  <Link to={path} className="flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all hover:bg-white/10 group">
...
export default function Sidenav() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-[260px] bg-gray-950 border-r border-white/5 flex flex-col justify-between py-8 px-6 text-white shadow-2xl">
      <div>
        {/* Top Section */}
        ...
        <nav className="space-y-1">
          {navItems.map((item, idx) => (
            <NavItem key={idx} label={item.label} icon={item.icon} path={item.path} />
          ))}
        </nav>
      </div>
      ...
      <div className="pt-6 border-t border-white/10">
        <Link to="/profile" className="flex items-center gap-4 px-2 py-2 hover:bg-white/5 rounded-lg transition-colors cursor-pointer">
          <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 p-0.5 shadow-md">
            <img 
          ...
          <div>
            <h3 className="text-sm font-semibold text-white">Sanjana B</h3>
